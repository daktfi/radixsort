...
/// Enable this if you need int128 support and your compiler doesn't provide is_integral for them.
#if 0
namespace std
{
...
/// Returns number of bits to sort (position of highest set bit).
template <typename T>
int significant_bits( const T &v );

/// Probably need to specifically exclude whatever very wide integer gonna use...
template <typename T>
typename std::enable_if<!std::is_integral<T>::value, int>::type significant_bits( const T & )
{
	return 8 * sizeof( T );
}
...
typename std::enable_if<!std::is_same<T, int16_t>::value && !std::is_same<T, uint16_t>::value
                            ...
                            && !std::is_same<T, uint128_t>::value && std::is_integral<T>::value,
                        int>::type
...
private:
	...
	static void sort_key_real( Container<Key> &keys )
	{
		...
		for( unsigned pass = 0, shift = 0; pass < passes_count; ++pass ) {
			Key *k0 = &keys[0], *k1 = &spare[0], *end = k0 + length;
			...
			if( !stat2offs( offsets, stat_length, length ) ) {
				...
				for( ; k0 < e1; ++k0 ) {
					__builtin_prefetch( offsets + ( ( k0[pf_solo_cnt] >> shift ) & digit_mask ), 1,
					                    3 );
	...
	static void sort_both_real( Container<Key> &keys, Container<Data> &vals )
	{
		...
		for( unsigned pass = 0, shift = 0; pass < passes_count; ++pass ) {
			...
			if( !stat2offs( offsets, stat_length, length ) ) {
				pass_both<Data, digit_width>( offsets, &keys[0], &keys[length], &vals[0],
				                              &spare_keys[0], &spare_vals[0], shift );
	...
	static void sort_data_real( const Container<Key> &keys, Container<Data> &vals )
	{
		...
		// Count statistics. Unless it's 8-bit key it fractional, so no reason to overoptimize.
		for( size_t row = 0; row < length; ++row )
			Counter<Key, digit_width, passes_count>::count( keys[row], stat );
		...
		if( real_passes > 1 ) {
			pass_both<Data, digit_width>(
			    stat_ptr[pass_number[pass]], key_source, key_source + length, vals.begin(),
			    key_target, spare_vals.begin(), digit_width * pass_number[pass], digit_mask );
